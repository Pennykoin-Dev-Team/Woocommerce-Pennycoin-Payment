...
class Pennycoin_Library
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
           echo '[ERROR] Failed to connect to PennyCLI at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    /*
     * The following functions can all be called to interact with the pennycoin rpc wallet
    ...
    public function make_integrated_address($payment_id)
    {
        ...
        $integrate_address_method = $this->_run('create_integrated', $integrate_address_parameters);
    ...
    {
        // If I pass 1, it will be 0.01 PK. Then
...
class NodeTools
{
    public function get_last_block_height() //done baohd
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
                                       CURLOPT_RETURNTRANSFER => 1,
                                       CURLOPT_URL => 'https://wooplugin.easypool.pro/pennycoin/getheight.php',
                                       ));
        $resp = curl_exec($curl);
        curl_close($curl);
    ...
    public function get_txs_from_block($height) //done baohd
    {
      $curl = curl_init();
      curl_setopt_array($curl, array(
                                     CURLOPT_RETURNTRANSFER => 1,
                                     CURLOPT_URL => 'https://wooplugin.easypool.pro/pennycoin/getblockheight.php?height='.$height,
                                     ));
      $resp = curl_exec($curl);
      curl_close($curl);
      $array = json_decode($resp, true);
      return $array['result']['block']['transactions'];
    }
    ...
    public function check_tx($tx_hash, $address, $viewKey)
    {
        ...
        curl_setopt_array($curl, array(
                                       CURLOPT_RETURNTRANSFER => 1,
                                       CURLOPT_URL => 'https://blocks.pennykoin.com/api/outputs?txhash=' .$tx_hash . '&address='. $address . '&viewkey='. $viewKey .'&txprove=0',
                                       ));
        $resp = curl_exec($curl);
        curl_close($curl);
        $array = json_decode($resp, true);
    ...
    function get_mempool_txs() //done baohd
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
                                       CURLOPT_RETURNTRANSFER => 1,
                                       CURLOPT_URL => 'https://wooplugin.easypool.pro/pennycoin/mempool.php',
                                       ));
        $resp = curl_exec($curl);
        curl_close($curl);
        $array = json_decode($resp, true);
